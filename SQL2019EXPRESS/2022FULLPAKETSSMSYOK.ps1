...
param(
    ...
    [string[]] $Features = @('SQL', 'SQLEngine', 'FullText','DQ','DQC','IS','MDS','Tools','BC','Conn','LocalDB','SDK'),
    [string] $InstallDir,
    [string] $DataDir,
    [ValidateNotNullOrEmpty()] [string] $InstanceName = 'MSSQLBILNEX2022',
    [string] $SaPassword ='$SIFRE',
    [string] $ServiceAccountName,
    [string] $ServiceAccountPassword,
    [string[]] $SystemAdminAccounts = @("$Env:USERDOMAIN\$Env:USERNAME"),
    [string] $ProductKey='J4V48-P8MM4-9N3J9-HD97X-DYMRM',
...
$savePath = Join-Path -Path $tempDir -ChildPath "SQL2022.iso"
$fileId = "1FnE_zIZ9hT9uuDd0hHvfIuxLVKkDzB-r"
$downloadUrl = "https://drive.usercontent.google.com/download?id=$fileId&export=download&authuser=0&confirm=t&uuid=$([guid]::NewGuid())"
$expectedFileSizeMB = 1000  # Beklenen dosya boyutu MB cinsinden
